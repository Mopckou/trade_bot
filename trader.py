...
class TRADER():
    ...
    def create_order_of_buy(self, important=True):
        ...
        if self.is_currency_for_buy(self.quantity_cash_of_buy):
            self.last_order_id = self._create_order_of_buy(price)
        else:
            if important:
                raise ErrorEnoughCash('Недостаточно валюты для совершения покупки!')

    def _create_order_of_buy(self, price):
        order_create_setup = {'pair': self.pair,
                              'quantity': self.quantity_cash_of_buy/price,
                              'price': price,
                              'type': 'buy'
                              }
    ...
    def __get_amount_by_last_orders(self, orders):
        ...
        self.logging(u'Сумма денег, которые были потрачены за последние ордера (до первой покупки) - %s' % amount)
        return amount
    ...
    def __get_quantity_by_last_orders(self, orders):
        ...
        quantity = quantity - (quantity * self.percent_of_burse) # с вычетом процента комиссии
        self.logging(u'Сумма валюты что мы купили за последние ордера - %s' % quantity)
        return quantity
